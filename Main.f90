...
subroutine ReadInput()
    ...
    open(unit=99,file='SMD.in')
        ...
        read(99,*)
        read(99,*)SMDEvolutionOrder
        read(99,*)
        read(99,*)BasisOrder
        read(99,*)
        read(99,*)PotentialOrder
        allocate(PolyPotentialCoeff(PotentialOrder))
        read(99,*)
        read(99,*)PolyPotentialCoeff
        read(99,*)
        read(99,*)advance
    close(99)
    if(advance) then
...
subroutine Initialize()
    ...
    if(JobType=='SMD') then!Allocate dynamics work space, provide initial value
        ...
        wigcoeff(1).miuq=q0
        wigcoeff(1).miup=p0
        wigcoeff(1).sigmaq=sigmaq
        wigcoeff(1).sigmap=sigmap
        wigcoeff(1).cor=0d0
        wigcoeff(1).b=0d0
        wigcoeff(1).b(1)=1d0
        InitialPurity=1d0
